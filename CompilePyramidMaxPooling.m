...
for f = 1:length(imageFileList)
    ...
    for l = 2:params.pyramidLevels
        ...
        for i=1:num_bins
            for j=1:num_bins
                % Max Pooling
                for k=1:params.dictionarySize
                    pyramid_cell{l}(i,j,k) = max([pyramid_cell{l-1}(2*i-1,2*j-1,k), pyramid_cell{l-1}(2*i,2*j-1,k), pyramid_cell{l-1}(2*i-1,2*j,k), pyramid_cell{l-1}(2*i,2*j,k)]);
                end
                % End Max Pooling
            end
        end
