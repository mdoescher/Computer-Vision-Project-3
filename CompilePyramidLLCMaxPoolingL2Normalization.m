function [ pyramid_all ] = CompilePyramidLLCMaxPoolingL2Normalization( imageFileList, dataBaseDir, textonSuffix, params, canSkip, pfig )
...
for f = 1:length(imageFileList)
    ...
    data=zeros(size(texton_ind.data,1),params.dictionarySize);
    for i=1:size(texton_ind.data,1)
        data(i,texton_ind.knn(i,:))=texton_ind.data(i,:);
    end
    %H=max(data);
    ...
    for i=1:binsHigh
        for j=1:binsHigh
            ...
            texton_patch = data( (texton_ind.x > x_lo) & (texton_ind.x <= x_hi) & ...
                                            (texton_ind.y > y_lo) & (texton_ind.y <= y_hi),:);
            
            % make histogram of features in bin
            r=max(texton_patch);
            r=r./norm(r);
            pyramid_cell{1}(i,j,:) = r;
        end
    end
    ...
    for l = 2:params.pyramidLevels
        ...
        for i=1:num_bins
            for j=1:num_bins
                pyramid_cell{l}(i,j,:) = max([pyramid_cell{l-1}(2*i-1,2*j-1,:); pyramid_cell{l-1}(2*i,2*j-1,:);pyramid_cell{l-1}(2*i-1,2*j,:);pyramid_cell{l-1}(2*i,2*j,:)]);
             end
        end
        num_bins = num_bins/2;
    end
